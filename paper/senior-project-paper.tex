...
Deep learning is often semi-supervised. We present a basic architecture consisting of 3 total layers based on an autoencoder and a logistic classifier. This will form the basis for constructing a deep neural net with more hidden layers.

One of the biggest hurdles to machine learing is the scarcity of labeled data. Every data point has to be manually labeled by humans - not every picture of a cat on the web is already tagged with ``cat''. Therefore, the large trove of data today mostly remains unusable by supervised machine learners. This necessitates new approaches - self-taught and unsupervised learning.


Given an unlabeled data set $\{x^{(1)}_u, x^{(2)}_u, \cdots, x^{(n)}_u \}$, perform self-taught learning on an autoencoder. This is called pretraining, which learns and encode the structure of the data in the hidden layers.

, the autoencoder sets the outputs $y^{(i)} = x^{(i)}_u$, and train on the data $\{(x^{(1)}_u, y^{(1)}), \cdots, (x^{(n)}_u, y^{(n)}) \}$
...
\caption{Supervised learning: Training Logistic Classifier using the activation units from the Sparse Autoencoder plus a bias unit. \href{http://deeplearning.stanford.edu/wiki/index.php/Self-Taught_Learning_to_Deep_Networks}{\tt Image from UFLDL.}}\label{LogisClass-fig}
\end{minipage}
\end{figure}
