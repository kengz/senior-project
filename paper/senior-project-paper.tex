...
Typically, an inner product is sufficient to yield a good ANN. For this we have the form $$\hat{V}_{i+1,j} = < w_{i,1,j}, \cdots, w_{i,p,j} > \cdot < n_{i,1}, \cdots, n_{i,p} > = w_{i,1,j}n_{i,1} + \cdots + w_{i,p,j}n_{i,p}$$
...
An explicit example is provided in Figure \ref{ANN-fig}. The feature vector input is $b = <0,0,1>$; the value $$n_{2,1} = \hat{V}_{2,1} = w_{1,1,1}n_{1,1} + w_{1,2,1}n_{1,2} + w_{1,3,1}n_{1,3} = 0.5 \cdot 0 + 0.4 \cdot 0 + 0.1 \cdot 1 = 0.1$$
...
The training rule for the algorithm is a simple update, $$w_i \leftarrow w_i + \eta(t-o)n_i$$ where $\eta$ is the stepsize moderation, $t$ is the target output value, $o$ the actual perceptron output, $n_i$ the input node value.
